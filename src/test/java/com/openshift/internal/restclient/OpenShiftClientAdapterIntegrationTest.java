 ...
 */
public class OpenShiftClientAdapterIntegrationTest {
...
//	}
	...
	public void testResourceLifeCycle() throws MalformedURLException {
		...
		Service service = (Service) factory.create(VERSION, ResourceKind.SERVICE);
		...
		Service otherService = (Service) factory.create(VERSION, ResourceKind.SERVICE);
		...
		try{
			...
			LOG.debug(String.format("Updating service: %s", otherService));
			otherService.setTargetPort(9999);
			otherService = client.update(otherService);
			LOG.debug(String.format("Updated service: %s", otherService));
